        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            @foreach ($articulos as $articulo)
                <a href="{{ route('articulos.show', $articulo) }}" class="group">
                    <div class="card-hover flex h-full flex-col overflow-hidden">
                        ...
                        @else
                            <div
                                class="from-accent/20 to-cream -mx-6 -mt-6 mb-4 flex h-48 items-center justify-center bg-gradient-to-br"
                            >
                                <span class="text-5xl opacity-50">📦</span>
                            </div>
                        @endif

                        <h3 class="text-thistle group-hover:text-fulvous mb-2 text-lg font-bold transition-colors">
                            {{ $articulo->nombre }}
                        </h3>

                        @if ($articulo->descripción)
                            <p class="text-charcoal/70 line-clamp-3 flex-1 text-sm">
